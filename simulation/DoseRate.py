...
class DoseRateFromDVH:
    ...
    """
    def __init__(self, n_fractions, total_field_on_time):
        self.n_fractions = n_fractions
        self.total_field_on_time = total_field_on_time

    def get_dose_rate_hist(self, dvh_file):
        dvh = pd.read_csv(dvh_file, header=0)
        dose_rate_bins = dvh['dose_bins'].values / self.n_fractions / self.total_field_on_time
