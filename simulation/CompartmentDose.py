...
# ======== Modified by Marina Garcia-Cardosa in February 2024 to manage with patient-specific compartments ========
#
import copy
import numpy as np
import numbers
...
class CompartmentDose:
    ...
    def dose_from_patient_specific_compartments(self, model):
        for i in range(len(self.dose)):
            self.dose[i] = model.patient.Particles[i].Dose
    ...
    def add_dose(self, dose_rate, compartment_ids, start_time, field_on_time):
        """
        ...
        assert (start_time + field_on_time <= self.blood_path.shape[1] * self.dt)
        ...
        idx1 = int(np.ceil((start_time + field_on_time) / self.dt))
